# 🔥 RESTART VAULT — Code Clean à copier-coller

## 1. SUPPRIMER toutes les routes /vault/* dans `/supabase/functions/server/index.tsx`

Remplacer les lignes 1367-2354 par ce code :
...
- GET `/vault` → récupère
- PUT `/vault` → sauvegarde
- POST `/vault/scan-url` → scrape (Jina) + analyse (Mistral)

Plus de complexité, plus de routes multiples, plus de fonctions helpers cachées qui crashent.

**Test** :
- Clique sur "Test" dans VaultPage

import{c as d}from"./index-DS3rtc7i.js";/**
 * @license lucide-react v0.487.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const f=[["circle",{cx:"12",cy:"12",r:"10",key:"1mglay"}],["path",{d:"m4.9 4.9 14.2 14.2",key:"1m5liu"}]],g=d("ban",f);function c(e){return e>=80?"#22c55e":e>=60?"#84cc16":e>=40?"#f59e0b":"#ef4444"}function x(e,i){return e>=80?"Excellent":e>=60?i?"Bon":"Good":e>=40?i?"Médiocre":"Mediocre":i?"Insuffisant":"Poor"}function o(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function a(e,i,n){const p=c(i.score),l=i.positives.map(t=>`<li>${o(t)}</li>`).join(""),s=i.issues.map(t=>`<li>${o(t)}</li>`).join("");return`
    <div class="kpi">
      <div class="kpi-header">
        <span class="kpi-label">${o(e)} <span class="kpi-weight">${n}</span></span>
        <span class="kpi-score" style="color:${p}">${i.score}<span class="kpi-max">/100</span></span>
      </div>
      <div class="kpi-bar"><div class="kpi-fill" style="width:${i.score}%;background:${p}"></div></div>
      ${l?`<div class="kpi-section"><div class="kpi-section-title positive">✓ Forces</div><ul>${l}</ul></div>`:""}
      ${s?`<div class="kpi-section"><div class="kpi-section-title issue">⚠ À améliorer</div><ul>${s}</ul></div>`:""}
    </div>
  `}function v(e,i){const n=c(e.overall),p=x(e.overall,i),l=`<!DOCTYPE html>
<html lang="${i?"fr":"en"}">
<head>
<meta charset="UTF-8" />
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .overall-score { font-size: 48px; font-weight: 900; line-height: 1; color: ${n}; }
  .overall-label { font-size: 14px; color: #666; }
  .overall-grade { font-size: 16px; font-weight: 700; color: ${n}; }
...
<body>
  ...
  <h2>${i?"Détail des 3 KPIs":"3 KPIs breakdown"}</h2>
  ${a("Legal",e.legal,"30%")}
  ${a("Brand Fit",e.brandFit,"35%")}
  ${a(i?"Créatif":"Creative",e.creative,"35%")}

  <div style="margin:16px 0;padding:12px 16px;border-radius:8px;font-weight:700;font-size:14px;display:inline-block;background:${e.publishVerdict==="safe"?"#dcfce7":e.publishVerdict==="revise"?"#fef3c7":"#fef2f2"};color:${e.publishVerdict==="safe"?"#15803d":e.publishVerdict==="revise"?"#b45309":"#b91c1c"}">
    Verdict: ${e.publishVerdict==="safe"?i?"Publier":"Publish":e.publishVerdict==="revise"?i?"À retoucher":"Revise":i?"Bloquer":"Block"}
  </div>

  ${e.recommendations.length>0?`
    <div class="recos">
      <div class="recos-title">${i?"Recommandations prioritaires":"Priority recommendations"}</div>
      <ol>${e.recommendations.map(r=>`<li><strong>[${o(r.kpi.toUpperCase())}]</strong> ${o(r.text)}${r.impact==="high"?" ⚠":""}</li>`).join("")}</ol>
    </div>
  `:""}
...
</html>`,s=new Blob([l],{type:"text/html"}),t=URL.createObjectURL(s);if(!window.open(t,"_blank")){const r=document.createElement("a");r.href=t,r.download=`ora-analyze-${e.id||Date.now()}.html`,r.click()}setTimeout(()=>URL.revokeObjectURL(t),6e4)}export{g as B,v as d};
